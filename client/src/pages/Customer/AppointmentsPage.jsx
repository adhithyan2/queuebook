        ...
        {tabs.map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 ${
              tab === t ? 'bg-primary text-white shadow-sm' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-600 hover:border-primary/30 hover:text-primary'
            }`}>
            {t}
          </button>
        ))}
      ...
      <div className="space-y-3">
        {filtered.length > 0 ? filtered.map((apt, i) => (
          <motion.div key={apt._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            className="flex items-center gap-4 p-5 bg-white dark:bg-slate-800 rounded-[20px] border border-slate-100 dark:border-slate-700 card-shadow card-shadow-hover cursor-pointer">
