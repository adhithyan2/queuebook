  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1 text-sm">Manage your appointments and bookings.</p>
      </div>
        ...
        {tabs.map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 ${
              tab === t ? 'bg-primary text-white shadow-sm' : 'bg-white text-slate-500 border border-slate-200 hover:border-primary/30 hover:text-primary'
            }`}>
            {t}
          </button>
        ))}
          ...
          <motion.div key={apt._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                <span className="flex items-center gap-1 text-xs text-slate-400"><HiOutlineCalendar className="w-3 h-3" /> {new Date(apt.date).toLocaleDateString()}</span>
                <span className="flex items-center gap-1 text-xs text-slate-400"><HiOutlineClock className="w-3 h-3" /> {apt.timeSlot}</span>
