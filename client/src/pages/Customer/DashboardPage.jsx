      ...
      <div className="space-y-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Here's what's happening with your queues today.</p>
          </div>
          <div className="relative w-full sm:w-auto">
            ...
            <input
              ...
              className="w-full sm:w-72 h-12 pl-11 pr-4 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
            />
          </div>
        </div>
              ...
              <Link to="/customer/appointments" className="block bg-white rounded-[20px] border border-slate-100 p-6 card-shadow-lg card-shadow-hover transition-all">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-slate-800">{upcomingAppointment.business?.name}</h3>
                    <p className="text-xs text-slate-500 mt-1">{upcomingAppointment.service}</p>
                    <div className="flex items-center gap-3 mt-3 flex-wrap">
                      <span className="flex items-center gap-1 text-xs text-slate-400"><HiOutlineCalendar className="w-3 h-3" /> {new Date(upcomingAppointment.date).toLocaleDateString()}</span>
                      <span className="flex items-center gap-1 text-xs text-slate-400"><HiOutlineClock className="w-3 h-3" /> {upcomingAppointment.timeSlot}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {nearbyBusinesses.slice(0, 3).map((biz) => (
                <div key={biz._id} className="bg-white rounded-[20px] border border-slate-100 p-6 card-shadow card-shadow-hover cursor-pointer transition-all">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 mb-1">{biz.name}</h3>
                  <p className="text-xs text-slate-400 capitalize mb-3">{biz.category}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      <HiOutlineStar className="w-3.5 h-3.5 text-amber-400 fill-current" />
                      <span className="text-xs font-semibold text-slate-700">{biz.rating?.toFixed(1) || 'New'}</span>
                    </div>
            ...
            {recentAppointments?.length > 0 ? (
              <div className="space-y-3">
                {recentAppointments.slice(0, 4).map((apt) => (
                  <div key={apt._id} className="flex items-center gap-4 p-4 rounded-xl bg-slate-50/80 hover:bg-slate-50 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center flex-shrink-0 border border-slate-100">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{apt.service} &middot; {new Date(apt.date).toLocaleDateString()}</p>
                    </div>
