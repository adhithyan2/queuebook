  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{business?.name || 'Business Dashboard'}</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Manage your queue and customer flow.</p>
        </div>
      ...
      <div className="bg-white rounded-[20px] border border-slate-100 dark:bg-slate-900 dark:border-slate-800 p-6 card-shadow mb-8">
        ...
        {queue.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No queue entries today</p>
          </div>
        )}
      </div>
            ...
            <div className="space-y-3">
              {analytics.analytics.slice(0, 7).map((day) => (
                <div key={day._id} className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 w-8 text-right">{day.count}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-10">No data available yet</p>
          )}
        </div>
            ...
            <div key={r._id} className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{r.user?.name}</p>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{r.comment || 'No comment'}</p>
              </div>
            </div>
          )) : (
